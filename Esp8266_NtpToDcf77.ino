...
Modified by Peter Gautier  4 May 2020
...
Ticker DcfOutTimer;  //routine timer 100 msec


Ticker pulseTimer;  //pulse time 1000msec
#define REFRESH 3600000ul   //in millisec

#define LedPin 2     //use GPIO numbers!!
#define LedPin2 13

//1 Hz pulse on a the following pin
#define pulsePin -1   //(D6)  if -1, not used!

#define pulseMode 0   // 0/1  if 1, mirror pulse
unsigned long int pulseRef = 0;   //reference pulse in msec (last sync sec)
...
void setup(){
  ...
  digitalWrite(LedPin, HIGH); 	digitalWrite(LedPin2, HIGH);
  Serial.print("DCF signal pins: GPIO"); Serial.print(LedPin); Serial.print(" and GPIO"); Serial.println(LedPin2);
  if (pulsePin>=0) { 
    pinMode(pulsePin, OUTPUT); 
    Serial.print("1 Hz pulse on pin GPIO"); Serial.println(pulsePin);
  }
  //handle DCF pulses
  DcfOutTimer.attach_ms(100, DcfOut);
  if (pulsePin>=0) pulseTimer.attach_ms(1000, pulse);
...
void ReadAndDecodeTime() {
  ...
  if (!cb) {
    ...
    //begin
    Serial.println("Sending DCF packets...");
    DCFOutputOn = 1;
    ...
    switch (PartialPulseCount++) {
      case 0:
        if (PulseArray[PulseCount] != 0)
          digitalWrite(LedPin, 1);  digitalWrite(LedPin2, 1);
        break;
      case 1:
        if (PulseArray[PulseCount] == 1)
          digitalWrite(LedPin, 0);  digitalWrite(LedPin2, 0);
        break;
      case 2:
        digitalWrite(LedPin, 0);  digitalWrite(LedPin2, 0);
        break;
...
static boolean newState = false;
  ...
  newState = ((millis()-pulseRef) / 1000) % 2;
  if (oldState !=newState) {
    if (pulseMode)
      digitalWrite(pulsePin, newState);
    else  
      digitalWrite(pulsePin, !newState);
